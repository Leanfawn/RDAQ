...
@register()
class RDAQCriterion(nn.Module):
    """ This class computes the loss for RDAQ.
    """
    ...
    def loss_dgre(self, outputs, targets, indices, num_boxes):
        assert 'pred_bbox_number' in outputs   
        dgre_targets = []   
        for i in range(len(targets)):     
            tgt_num = targets[i]['labels'].shape[0]
            t = 0
            for j in range(len(self.dgre_params)):
                if tgt_num >= self.dgre_params[j]:     
                    t = j + 1
            dgre_targets.append(t)   
        dgre_targets = torch.tensor(dgre_targets, dtype=torch.int64).to(outputs['pred_bbox_number'].device)
    
        loss = torch.nn.CrossEntropyLoss()(outputs['pred_bbox_number'], dgre_targets)   
   
        return {'loss_dgre': loss}
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        if 'aux_outputs' in outputs:    
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:  
                    if loss in ['dgre', 'densitymap', 'densitycount']:    
                        continue
                    # TODO, indices and num_box are different from RT-DETRv2    
                    use_uni_set = self.use_uni_set and (loss in ['boxes', 'local'])   
